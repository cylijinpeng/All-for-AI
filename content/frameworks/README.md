# Frameworks

Major agent, orchestration, retrieval, and workflow frameworks used by AI builders.

## Starter entries

- [`openai-agents-python`](openai-agents-python/README.md)
- [`openai-agents-js`](openai-agents-js/README.md)
- [`microsoft-autogen`](microsoft-autogen/README.md)
- [`langgraph`](langgraph/README.md)
- [`pydantic-ai`](pydantic-ai/README.md)
- [`crewai`](crewai/README.md)
- [`llamaindex`](llamaindex/README.md)
- [`dspy`](dspy/README.md)
- [`promptflow`](promptflow/README.md)
- [`litellm`](litellm/README.md)
- [`semantic-kernel`](semantic-kernel/README.md)
- [`smolagents`](smolagents/README.md)
- [`mastra`](mastra/README.md)
- [`agno`](agno/README.md)
- [`haystack`](haystack/README.md)
